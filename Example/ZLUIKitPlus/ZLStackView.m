...
@interface ZLStackView ()
@property (nonatomic,strong)ZLSpacingGuideCoordinator *guideCoordinator;
@end
@implementation ZLStackView

- (ZLSpacingGuideCoordinator *)guideCoordinator {
    if (!_guideCoordinator) {
        _guideCoordinator = [[ZLSpacingGuideCoordinator alloc] init];
        _guideCoordinator.stackView = self;
    }
    return _guideCoordinator;
}
- (NSMutableArray<__kindof UIView *> *)arrangedViews {
    if (!_arrangedViews) {
        _arrangedViews = NSMutableArray.array;
...
- (void)updateConstraints {
    [super updateConstraints];
    [self.guideCoordinator addHorizontalConstraint];
    [self.guideCoordinator addWidthConstraint];
    [self.guideCoordinator activateConstraints];
}
@end
